...
class NPUExecutor(ExecutorBase):
    ...
    def _init_executor(self) -> None:
        assert (self.lora_config is
                None), "LoRA is not supported for NPU backend."
        assert (not self.speculative_config
                ), "Speculative decoding not yet supported for NPU backend."
    ...
    def execute_model(
        ...
        assert (not execute_model_req.blocks_to_swap_in
                ...
                and not execute_model_req.blocks_to_copy), (
                    "Cache operations are not supported for NPU backend.")
        assert execute_model_req.num_lookahead_slots == 0, (
            "lookahead not supported for NPU backend.")
    ...
    def add_prompt_adapter(self, prompt_adapter_request) -> bool:
        raise NotImplementedError(
            "Soft prompt is currently not supported by the NPU backend.")
    ...
    def check_health(self) -> None:
        # NPUExecutor will always be healthy as long as
        # it's running.
        return
